...
gosu "$run_uid:$run_gid" node apps/daemon/dist/cli.js --no-open --host 127.0.0.1 --port "$internal_port" &
daemon_pid="$!"

gosu "$run_uid:$run_gid" socat "TCP-LISTEN:${external_port},fork,reuseaddr,bind=0.0.0.0" "TCP:127.0.0.1:${internal_port}" &
proxy_pid="$!"
